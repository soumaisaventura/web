SELECT migrar ();
  ...
  SELECT DISTINCT questao_id, questao
...
SELECT _ru2.usuario_id
 ...
 WHERE     _ru2.usuario_id IN
       ...
       AND _ru2.new_resposta_id = 1
INTERSECT
SELECT usuario_id FROM lista_negra;
...
SELECT * FROM respostas_count;



SELECT count (DISTINCT usuario_id) FROM resposta_usuario WHERE questao_id = 16 AND new_resposta_id IN (0);

SELECT count (DISTINCT usuario_id) FROM resposta_usuario;
...
SELECT min (data), max (data) FROM tabelao;