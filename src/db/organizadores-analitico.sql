...
AS
  SELECT
    ...
    p.name                                        AS user_name,
    CASE WHEN p.gender LIKE 'MALE'
      THEN 'M'
    ELSE 'F' END                                  AS user_gender,
...
SELECT
  v.race_city_name || '/' || v.race_state_abbreviation AS cidade,
  count(*)                                             AS qtd
FROM vw_user_registration v
...
SELECT
  v.race_city_name || '/' || v.race_state_abbreviation,
  sum((CASE WHEN v.category_team_size = 4
    THEN 1
       ELSE 0 END))
    AS quartetos,
  sum((CASE WHEN v.category_team_size = 2
    THEN 1
       ELSE 0 END))
    AS duplas
FROM vw_user_registration v
...
SELECT
  sum(aux1.desconto) / 110.0 * 100 AS desconto,
...
SELECT
  ...
  sum(aux2."9")     AS "9"
FROM (SELECT
        CASE WHEN aux1.qtd = 1
          THEN 1
        ELSE 0 END AS "1",
        CASE WHEN aux1.qtd > 1 AND aux1.qtd <= 3
          THEN 1
        ELSE 0 END
                   AS "2 - 3",
        CASE WHEN aux1.qtd > 3 AND aux1.qtd <= 5
          THEN 1
        ELSE 0 END
                   AS "4 - 5",
        CASE WHEN aux1.qtd > 5 AND aux1.qtd <= 8
          THEN 1
        ELSE 0 END
                   AS "6 - 8",
        CASE WHEN aux1.qtd = 9
          THEN 1
        ELSE 0 END AS "9"
      FROM (SELECT
              v.user_id,
