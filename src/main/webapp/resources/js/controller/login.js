...
function login() {
	...
	proxy.login(form, loginOk, loginFailed);
}
...
function facebookLogin(response) {
	if (response.authResponse) {
		...
		var proxy = new AuthProxy();
		proxy.facebookLogin(response.authResponse.accessToken, facebookLoginOk,
				facebookLoginFailed);
...
function googleLogin(authResult) {
	if (authResult['access_token']) {
		$("#form-login input").attr("disabled", true);
		var proxy = new AuthProxy();
		proxy.googleLogin(authResult.code, googleLoginOk);
