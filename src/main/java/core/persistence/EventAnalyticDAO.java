...
import static core.entity.RegistrationStatusType.CONFIRMED;
...
public class EventAnalyticDAO extends JPACrud<Race, Integer> {
    ...
    public List<EventAnalytic> getRegistrationByCategory(Event event) {
        String jpql = "";
        jpql += " select ";
        jpql += "    new core.entity.EventAnalytic( ";
        jpql += "        ca.name ";
        jpql += "        || ' ' || ra.name ";
        jpql += "        , ";
    ...
    public List<EventAnalytic> getAmountDiscounted(Event event) {
        ...
        jpql += "           cast(sum(pe.price) + sum(case when k is null then 0 else k.price end) - sum(ur.amount) as long) ";
        jpql += "        ) ";
        jpql += "   from UserRegistration ur ";
